...
final class CardInteractor: CardInteractorProtocol {
    ...
    private func generateWildCards() {
        wildCards = [
            ...
            addCards(Card(title: "Steal Wild Card", description: "Steal a random wild card from a player", effect: .stealWild), count: 2),
            addCards(Card(title: "Team Up", description: "Team up for a challenge with a player of your choice", effect: .teamUp), count: 2)
        ].flatMap { $0 }.shuffled()
    }
